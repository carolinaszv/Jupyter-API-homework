...
#API key e6a6eff8d36ee43af21205f81569462e
...
url = "http://ws.audioscrobbler.com/2.0/?method=artist.search&artist=lil&limit=50&api_key=e6a6eff8d36ee43af21205f81569462e&format=json"
...
for lil_artist in lastfm_data['results']['artistmatches']['artist']:
    print("The artist",lil_artist['name'],"has",lil_artist['listeners'],"listeners.")
...
total_listeners = 0
for lil_artist in lastfm_data['results']['artistmatches']['artist']:
    total_listeners = int(lil_artist['listeners']) + total_listeners
print("My list has a total of",total_listeners,"listeners.")
...
for lil_artist in lastfm_data['results']['artistmatches']['artist']:
    for display_image in lil_artist['image']:
        if display_image['size'] == 'extralarge':
            print(lil_artist['name'],display_image['#text'])
...
for lil_artist in lastfm_data['results']['artistmatches']['artist']:
    if lil_artist['name'] == 'Lil Jon':
        print(lil_artist['name'], "has the mbid:", lil_artist['mbid'])
...
liljon_url = "http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&artist=LilJon&mbid=a95384b1-6aec-468c-ae0d-8c6daf87c4c2&api_key=e6a6eff8d36ee43af21205f81569462e&format=json"
response = requests.get(liljon_url)
...
print(liljon_data['artist']['name'], "-", liljon_data['artist']['bio']['summary'])
...
for liljon_tag in liljon_data['artist']['tags']['tag']:
    print(liljon_tag['name'])
...
mbid_url = []
for url in mbids:
    api_url = "http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&api_key=e6a6eff8d36ee43af21205f81569462e&format=json&mbid="
    mbid_url.append(api_url + url)
for result in mbid_url:
    print(result)
#without the second for loop, it just printed a list in brackets and I did not like that.
...
for result in mbid_url[:3]:
    ...
    mbid_data = response.json()
    tags = ""
    for mbid_tag in mbid_data['artist']['tags']['tag']:
        tags = tags + mbid_tag['name'] + ", "
    print(mbid_data['artist']['name'],tags)
...
rapper = ['rap', 'Hip-Hop', 'hip hop', 'Gangsta Rap', 'Dirty South', 'swag', 'crunk', 'drill', 'trap']
for result in mbid_url[:10]:
    response = requests.get(result)
    mbid_data = response.json()
    raps = 0
    for mbid_tag in mbid_data['artist']['tags']['tag']:
#        print(mbid_data['artist']['name'], mbid_tag['name'])
        if mbid_tag['name'] in rapper:
            raps = raps + 1
    if raps > 0:
        print(mbid_data['artist']['name'], "is a rapper.")
    else:
        print(mbid_data['artist']['name'], "is not a rapper.")
#count at zero should be outside of for loop otherwise it will drop back to zero during next loop
...
for result in mbid_url[:10]:
    ...
    for mbid_tag in mbid_data['artist']['tags']['tag']:
        if mbid_tag['name'] in rapper:
            raps = raps + 1
    if raps > 0:
        rapper_count.append(mbid_data['artist']['name'])
        #count = count + 1
print(len(rapper_count)/10*100,"percent are rappers.")
#print(count/10*100,"percent are rappers.")
